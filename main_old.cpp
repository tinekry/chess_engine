...
/*
struct A { std::vector<int> data = {1, 2}; };
struct B { std::vector<int> data = {3, 4}; };
struct C { std::vector<int> data = {5, 6}; };
struct D { std::vector<int> data = {7, 8}; };

// Наследуемся от любого количества классов через шаблон
template <typename... Bases>
struct Derived : public Bases... {
    
    // Метод для обхода всех векторов всех базовых классов
    void print_all() {
        // Fold expression (свертка): выполняет код для каждого типа из пакета Bases
        (..., [this]() {
            for (int val : Bases::data) {
                std::cout << val << " ";
            }
        }());
    }
